...
namespace BlazorShared.Helper
{
    public static partial class CommonHelper
    {
        ...
        /// <returns></returns>
        public static Dictionary<T, string> GetEnumModelHeader<T>(Type type)
        {
            ...
            if (enumFields != null && enumFields.Length >= 2)//枚举有一个隐藏的int所以要从下一位置开始
            {
                ...
                for (int i = 0; i < enumFields.Length; i++)
                {
                    ...
                    var enumName = enumFields[i].Name;
        ...
        public static bool ShowMessage<T>(this AdminCodeResult<T> admin,IPopupService popupService)
        {
            if (admin.Code == 200)
            {
                popupService.EnqueueSnackbarAsync("操作成功", BlazorComponent.AlertTypes.Success);
                return true;
            }
            else
            {
                popupService.EnqueueSnackbarAsync("操作失败"+admin.Message, BlazorComponent.AlertTypes.Error);
                return false;
            }
        }
        /// <summary>
        ...
        /// <returns></returns>
        public static T GetResult<T>(this AdminCodeResult<T> result) where T:class,new()
        {
            if(result.Code != 200)
            {
                return new ();
